...
from cookAIware.config import config
from cookAIware.prompts import get_session_voice, get_session_instructions
from cookAIware.app_settings import get_language
from cookAIware.tools.core_tools import (
    ToolDependencies,
    get_tool_specs,
    dispatch_tool_call,
)
...
class OpenaiRealtimeHandler(AsyncStreamHandler):
    ...
    async def apply_personality(self, profile: str | None) -> str:
        ...
        try:
            from cookAIware.config import config as _config
    ...
    async def apply_language(self, language: str) -> str:
        """Apply a new language to the realtime session if possible."""
        lang = (language or "").strip() or "en"
        try:
            instructions = get_session_instructions().rstrip()
            ...
            if self.connection is not None:
                try:
                    ...
                    return f"Language updated to {lang}."
                except Exception:
                    await self._restart_session()
                    return f"Language updated to {lang} (session restarted)."
            return f"Language set to {lang}. It will apply on next connection."
        except Exception as e:
            logger.warning("Failed to apply language: %s", e)
            return f"Failed to apply language: {e}"
    ...
    async def _run_realtime_session(self) -> None:
        ...
        async with self.client.realtime.connect(model=config.MODEL_NAME) as conn:
            try:
                language = "en"
                try:
                    if self.deps.data_dir is not None:
                        language = get_language(self.deps.data_dir)
                except Exception:
                    language = "en"
                ...
                await conn.session.update(
                    session={
                        "type": "realtime",
                        "instructions": f"{get_session_instructions().rstrip()}\n\nRespond in {language}.",
                        "audio": {
                            "input": {
                                "format": {
                                    "type": "audio/pcm",
                ...
                if not self.gradio_mode and not self._greeted:
                    try:
                        await conn.conversation.item.create(
                            item={
                                ...
                                "content": [
                                    {
                                        "type": "input_text",
                                        "text": "system_start: greet the family and confirm you are ready.",
                                    }
                                ],
                            }
                        )
                        await conn.response.create(
                            response={
                                "instructions": "Say hello in the configured language and confirm you are ready to help with inventory, meals, and shopping lists.",
                            }
                        )
                        self._greeted = True
                    except Exception:
                        logger.warning("Failed to send auto greeting.")
    ...
    async def send_text(self, text: str) -> bool:
        """Send a text input into the active realtime session."""
        if not self.connection:
            return False
        try:
            await self.connection.conversation.item.create(
            ...
            await self.connection.response.create(
                response={
                    "instructions": "Respond to the user's text input.",
                }
            )
            return True
        except Exception:
